...
- [ ] 1.11 Feasibility gate: run the PR workflow and record the GitHub Actions URL proving both `build-windows (amd64)` and `build-windows (arm64)` schedule and pass. If `windows-11-arm`, ARM64 Python, PyInstaller, or AGH dependencies fail, stop as blocked; do not proceed to PR 2 docs or advertise Scoop.
...
- [ ] 2.1 Add failing `tests/test_release_workflow.py` assertions that `.github/workflows/release.yml` contains `update-scoop`, depends on `github-release`, validates `SCOOP_BUCKET_TOKEN`, uses `giulianotesta7/scoop-agh`, edits only `bucket/agh.json`, hashes published GitHub Release assets, includes both `64bit` and `arm64`, uses branch `agh-${VERSION}`, and has no `continue-on-error`.
- [ ] 2.2 Add failing docs assertions in `tests/test_docs_guidance.py` that `README.md` and `README.es.md` include equivalent Scoop commands: `scoop bucket add agh https://github.com/giulianotesta7/scoop-agh` and `scoop install agh`, while preserving the existing README H2 lists.
- [ ] 2.3 Add failing changelog verification by planning `changelog.d/+scoop-windows-installer.added.md`; validate later with `uv run towncrier check` rather than editing `CHANGELOG.md`.

### GREEN

- [ ] 2.4 Modify `.github/workflows/release.yml` to add release-failing `update-scoop` after `github-release`: validate token, verify/download published `v${VERSION}` Windows assets, compute SHA256, clone `giulianotesta7/scoop-agh`, update `bucket/agh.json` version/URLs/hashes/bin, validate complete GitHub Release-backed metadata, push `agh-${VERSION}`, and create or update the bucket PR.
- [ ] 2.5 Preserve existing `update-homebrew`, PyPI, GHCR, and GitHub Release behavior while adding Scoop; do not change Homebrew's PyPI-based formula update flow.
- [ ] 2.6 After PR 1 feasibility evidence is recorded, update `README.md` under existing `## Install` to document Scoop as the Windows install path without adding a new H2.
- [ ] 2.7 Mirror the same user-facing Scoop guidance in `README.es.md` under existing `## Instalar`, keeping command semantics identical.
- [ ] 2.8 Create `changelog.d/+scoop-windows-installer.added.md` with one user-facing sentence announcing the new Scoop Windows installation channel.

### TRIANGULATE

- [ ] 2.9 Extend static assertions for failure semantics and data source: `update-scoop` must use GitHub Release asset URLs, must not reference PyPI or workflow-only artifacts for the manifest, must fail on missing hashes/metadata, and must not run before `github-release`.
- [ ] 2.10 Run and record focused evidence: `uv run pytest tests/test_release_workflow.py tests/test_docs_guidance.py -q`, `uv run towncrier check`, `git diff --check`, and `uv lock --locked`.
- [ ] 2.11 Before final review, run broader validation when practical: `uv run pytest -q`, `uv run --with ruff ruff check .`, `uv run --with ruff ruff format --check .`, and `uv run --with pyright pyright agh tests`.

### REFACTOR

- [ ] 2.12 Keep Scoop manifest mutation readable and reviewable in `.github/workflows/release.yml`; if the JSON edit/validation block becomes too large, extract a focused helper under `scripts/` with paired tests before continuing.
- [ ] 2.13 Record first real tag-release monitoring steps in the PR notes: verify GitHub Release assets, `update-homebrew`, generated `giulianotesta7/scoop-agh` PR, and expected red release status if `update-scoop` fails after publication.
